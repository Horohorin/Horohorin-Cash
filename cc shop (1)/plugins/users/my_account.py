        ...
        inline_keyboard=[
            [
             ...
             [
                 ...
                 InlineKeyboardButton("⚜️ Trocar Cashback", callback_data="swap"),
             ],
            # [
...
<i>Divulgue seu link de afiliado e ganhe 9% em toda recarga do seu indicado!</i>
    
<b>Como funciona?</b>
    
<i>- Cada pessoa que for indicada pelo seu link e recarregar qualquer valor, você vai ganhar 9% do valor que ele recarregou, isso inclui todas as recargas que ele fizer!</i>
...
async def buy_history(c: Client, m: CallbackQuery):
    ...
    history = cur.execute(
        "SELECT number, month, year, cvv, level FROM cards_sold WHERE owner = ? ORDER BY bought_date DESC LIMIT 50",
        [m.from_user.id],
    ).fetchall()
